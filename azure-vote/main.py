...
loghandler = AzureLogHandler(connection_string=os.getenv('app_insight'))
loghandler.setFormatter(logging.Formatter('%(traceId)s %(spanId)s %(message)s'))
eventhandler = AzureEventHandler(connection_string=os.getenv('app_insight'))
...
exporter = metrics_exporter.new_metrics_exporter(connection_string=os.getenv('app_insight'))
...
def index():
    ...
    if request.method == 'GET':
        ...
        if request.form['vote'] == 'reset':
            ...
            logger.info("Voted for cat", extra=properties)
            vote2 = r.get(button2).decode('utf-8')
            properties = {'custom_dimensions': {'Dogs Vote': vote2}}
            # TODO: use logger object to log dog vote
            logger.info("Voted for dog", extra=properties)
